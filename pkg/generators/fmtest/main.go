...
func Create(s qst.SurveyT) (*qst.QuestionnaireT, error) {
	...
	// page 2 - inflation + zinsen
	{
		...
		// gr2
		{
			...
			gb.MainLabel = trl.S{
				...
				"en": "<b>5.</b> &nbsp; In the medium-term (<bx>6</bx>&nbsp;months), <i>long-term</i> interest rates (yields on 10-year sovereign bonds) will",
			}
			gr := page.AddGrid(gb)
			gr.BottomVSpacers = 4
		}
	...
	err = special202511(&q)
	if err != nil {
		return nil, fmt.Errorf("error adding special202511(): %v", err)
	}
	...
	// err = special202603Climate(&q)
	// if err != nil {
	// 	return nil, fmt.Errorf("error adding special202603Climate(): %v", err)
	// }
	...
	err = special202609Climate(&q)
	if err != nil {
		return nil, fmt.Errorf("error adding special202609Climate(): %v", err)
	}
