...
import (
	...
	"github.com/zew/go-questionnaire/pkg/handler"
...
var fcNav = func(r *http.Request, q *qst.QuestionnaireT, SBookMarkURI string) template.HTML {
	...
	if q != nil && len(q.Pages) > 0 {
		...
		for i, lpP := range q.Pages {
			...
			onClick := fmt.Sprintf(
				`onclick="document.forms.frmMain.page.value='%v';document.forms.frmMain.submit(); return false;"`,
				i,
			)
			...
			ins := insertT{
				handler.Info{Title: title, Keys: []string{key}, Urls: []string{nodeURL}, OnClick: onClick},
				asChild,
			}
			if i == q.CurrPage {
				ins.Urls = []string{}
